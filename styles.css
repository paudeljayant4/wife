...
class ParticleHearts {
  constructor() {
    this.canvas = document.getElementById('particle-hearts-canvas');
    if (!this.canvas) return;
  ...
  setupEventListeners() {
    ...
    this.canvas.addEventListener('click', (e) => this.createHeartBurst(e.clientX, e.clientY));
  ...
  createHeartBurst(x, y) {
    const colors = CONFIG.particleHearts.colors;
    
    for (let i = 0; i < CONFIG.particleHearts.burstCount; i++) {
      const angle = (i / CONFIG.particleHearts.burstCount) * Math.PI * 2;
  ...
  drawHeart(x, y, size, rotation, alpha, color) {
    this.ctx.save();
    ...
    this.ctx.bezierCurveTo(-size * 0.5, -size * 0.3, -size * 0.8, size * 0.1, 0, size * 0.8);
    this.ctx.bezierCurveTo(size * 0.8, size * 0.1, size * 0.5, -size * 0.3, 0, size * 0.3);
    this.ctx.fill();
  ...
  animate() {
    ...
    this.hearts.forEach((heart, index) => {
      ...
      heart.x += heart.vx;
      heart.y += heart.vy;
      heart.vy -= 0.05; // Gravity (upward)
      heart.vx *= 0.98; // Air resistance
      heart.rotation += heart.rotationSpeed;
      heart.life -= 0.01;
      heart.alpha = heart.life;
      
      // Draw
      this.drawHeart(heart.x, heart.y, heart.size, heart.rotation, heart.alpha, heart.color);
